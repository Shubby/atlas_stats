...
void AtlasPapers::Loop()
{
   ...
   std::cout << "Number of articles is:" << narticoli << std::endl;
   ...
   for (Long64_t jentry=0; jentry<narticoli;jentry++) {
     ...
     for (unsigned int ied=0;ied<EditCountry->size();ied++) {
     ...
     for (unsigned int ied=0;ied<EdboCountry->size();ied++) {
   ...
   for (Long64_t jentry=0; jentry<narticoli;jentry++) {
     ...
     hname="HD"+Anno;     
     if (H1D.find(hname)!=H1D.end()) H1D[hname]->Fill(Groups[gruppo],EditCountry->size());
     hname="HDALL";     
     if (H1D.find(hname)!=H1D.end()) H1D[hname]->Fill(Groups[gruppo],EditCountry->size());
...
//	 std::cout<<Anno<<" "<<titolo<<" "<<EBName->at(ied)<<" EB member "<<std::endl;
     ...
     for (unsigned int ied=0;ied<EditCountry->size();ied++) {
       ...
       if (paese=="Italy") {
	 isItaly=true;
	 std::cout<<Anno<<" "<<titolo<<" "<<gruppo<<" "<<Status->at(0)<<" "<<std::endl;
       }
