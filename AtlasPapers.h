...
#ifndef AtlasPapers_h
#define AtlasPapers_h
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   Int_t           NEditors;
   Int_t           NEBoard;
   Int_t           NCountry[200];   //[NEditors]
   Int_t           NBCountry[200];   //[NEBoard]
   vector<string>  *Title;
   vector<string>  *Group;
...
#ifdef AtlasPapers_cxx
AtlasPapers::AtlasPapers(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("data/atlas_papers_2016.root");
      if (!f || !f->IsOpen()) {
...
AtlasPapers::~AtlasPapers()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t AtlasPapers::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t AtlasPapers::LoadTree(Long64_t entry)
{
...
void AtlasPapers::Init(TTree *tree)
{
...
void AtlasPapers::Show(Long64_t entry)
{
...
Int_t AtlasPapers::Cut(Long64_t entry)
{
