...
if not papers_per_Year.has_key("2015"):
    papers_per_Year["2015"]=0
    pass
...
for node_h in root.findall('row'):
    ...
    for i in  node_h.findall(".//column[@description='Publication Date']"):
        ...
        if my_year == "2015":
            print "adding to year oucnter",short_title, " date:", date_string
            papers_per_Year["2015"] += 1
            print "counter is: ", papers_per_Year["2015"]
        else:
            break;
        
    #full title
    for i in  node_h.findall(".//column[@description='Full Title']"):
        if i.text is None:
            print "Warning! Cannot find publication Full Titile for", short_title, " Skipping publication"
            break;
        string_to_print+=i.text+my_sep
    
    #Journal of final submission"
    for i in  node_h.findall(".//column[@description='Journal of final submission']"):
        if i.text is None:
            print "Warning! Cannot find publication Journal of final submissions for", short_title, " Skipping publication"
            break;
        string_to_print+=i.text+my_sep
    

    # All links
    for i in  node_h.findall(".//column[@description='All links']"):
        if i.text is None:
            print "Warning! Cannot find publication All links for", short_title, " Skipping publication"
            break;
        ...
        for il in list_of_links:
            ...
            if my_match:
                ...
                string_to_print+=doi_expr+my_sep
                break
    #arxiv
    for i in  node_h.findall(".//column[@description='arXiv Reference']"):
        if i.text is None:
            print "Warning! Cannot find publication Arxiv for", short_title, " Skipping publication"
            break;
        string_to_print+=i.text+my_sep
#    if my_year == "2015":
#        print "VVV",string_to_print
    # now loop on  contributor types
    for i_edtyp in EditorTypeList:
        for i in  node_h.findall(".//column[@description='"+i_edtyp+"']"):
...
for i_yea in YearList:
    outfile.write(i_yea+"|||||"+str(total_number_authors)+"|"+str(papers_per_Year["2015"])+"\n")
