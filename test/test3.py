...
for note_id in root.findall('.//{http://purl.org/dc/elements/1.1/}identifier'):
...
for node_h in .findall('{http://purl.org/dc/elements/1.1/}dc'):
    for node in node_h.findall('{http://purl.org/dc/elements/1.1/}creator'):
        #print node.tag," 1 ", node.attrib, " 2 ", node.text, " 3 ",node.tail
        #print "CAZZO", node.text
        auth_text = node.text; 
        if not re.match('.*Collaboration',auth_text):
            #print "CAZZO1", node.text
            if re.match('.*,.*',auth_text):
                #print "CAZZO2", node.text
            ...
            print "new_author=",new_auth
                #            for note_id in node.findall('{http://purl.org/dc/elements/1.1/}identifier'):
            #for note_id in node.findall('.{http://purl.org/dc/elements/1.1/}identifier'):
